...
var Mapa = (function () {
  ...
  /* alfinetes vivem fora da câmera: só a posição muda, o tamanho nunca.
     translate e scale vão no mesmo atributo para não brigar com a
     animação CSS de queda, que roda no <g> filho. */
  function layoutPins () {
    UNIDADES.forEach(function (u) {
      u.node.setAttribute('transform',
        'translate(' + screenX(u.x).toFixed(2) + ',' + screenY(u.y).toFixed(2) + ') scale(' + ps.toFixed(3) + ')');
    });
    clusterG.setAttribute('transform',
      'translate(' + screenX(SC_MAP.cluster.cx).toFixed(2) + ',' + screenY(SC_MAP.cluster.cy).toFixed(2) +
      ') scale(' + ps.toFixed(3) + ')');
  }
  ...
  function buildPin (u, i) {
    ...
    if (u.ox || u.oy) {
      g.appendChild(el('line', { class: 'pin__leader', x1: 0, y1: 0, x2: u.ox, y2: u.oy }));
      g.appendChild(el('circle', { class: 'pin__anchor-dot', cx: 0, cy: 0, r: 2.4 }));
    }

    var body = el('g', { class: 'pin__body', transform: 'translate(' + u.ox + ',' + u.oy + ')' });
    ...
    var label = el('text', { class: 'pin__label', x: u.ox, y: u.oy + (u.la ? -38 : 16) });
  ...
  function play () {
    stage.classList.remove('is-drawn', 'is-zoomed', 'is-pinned');
    setView(VIEW_STATE);
    badge.textContent = 'Santa Catarina';
    ...
    setTimeout(function () { stage.classList.add('is-pinned'); }, 2700);
  }

  function init () {
    if (typeof SC_MAP === 'undefined' || $('#mapaStage').dataset.ready) return;
    stage    = $('#mapaStage');
    ...
    window.addEventListener('resize', function () {
      clearTimeout(rt);
      rt = setTimeout(function () { measure(); layoutPins(); }, 150);
    }, { passive: true });
